...
async function execute(data, requestId, config) {
  ...
  logger.info('[Commodities Judge] Decisão para Commodities', { requestId });
  ...
  const preco = visao.preco_atual || 2000;
  ...
  return {
    ...
    justificativa: `Commodities: RSI ${rsi}, Score ${score}. SL ${slPercent}%, TP ${tpPercent}%.`,
    estrategia: {
      preco_atual: preco,
