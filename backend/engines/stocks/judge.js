...
async function execute(data, requestId, config) {
  ...
  logger.info('[Stocks Judge] Decisão para ações', { requestId });
  ...
  const preco = visao.preco_atual || 150.00;
  const slPercent = config.risk.default_sl_percent || 3;
  const tpPercent = config.risk.default_tp_percent || 6;
  ...
  return {
    ...
    justificativa: `Ações: RSI ${rsi}, Score ${score}. SL ${slPercent}%, TP ${tpPercent}%.`,
    estrategia: {
      preco_atual: preco,
